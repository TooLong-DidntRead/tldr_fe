...
const Results = ({ concerns }: ResultsProps) => {
  ...
  const ratingExplanation: string =
    "Consumer Friendliness Rating: the progress bar represents a rating from 1 - 10. 1 being the most friendly to the business which created the ToS, 10 being the most consumer-friendly.";

  const concernRows = concerns.map((concern, i) => {
    return (
      <ConcernRow
        id={i}
        key={i}
            ...
            <div className="results-card-header">
              ...
              <div className="meter-parent">
                ...
                <Tooltip title={ratingExplanation} tabIndex={0}>
                  <HelpOutlineIcon sx={{ color: "#0D4C92", fontSize: 16 }} />
                </Tooltip>
              </div>
            </div>
